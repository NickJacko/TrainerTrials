// assets/js/pages/catchdex_live_fx.js
(() => {
  ...
  if (cv && !reduced) {
    ...
    for (let i = 0; i < 50; i++) parts.push({
      ...
      vx: (Math.random() - 0.5) * 0.15, vy: -0.05 - Math.random() * 0.22,
      ph: Math.random() * Math.PI * 2, sp: 0.012 + Math.random() * 0.02,
      hue: Math.random() < 0.5 ? 175 : (Math.random() < 0.5 ? 48 : 280)
    });
